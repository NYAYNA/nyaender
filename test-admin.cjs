...
async function run(){const nodes=new Map();const get=id=>{if(!nodes.has(id))nodes.set(id,new Element());return nodes.get(id);};let mode='ok',puts=0,reads=0;const context={location:{hash:""},requestAnimationFrame:fn=>fn(),document:{getElementById:get,createElement:()=>new Element(),createTextNode:t=>t,querySelectorAll:()=>[]},window:{addEventListener(){}},TextEncoder,TextDecoder,Uint8Array,Date,Blob,URL,setTimeout,confirm:()=>true,btoa:s=>Buffer.from(s,'binary').toString('base64'),atob:s=>Buffer.from(s,'base64').toString('binary'),fetch:async(url,o)=>{
 ...
 const savedData=runCode('JSON.stringify(data)');
 runCode('data.sections=[{id:"test",title:"Test",cards:[{title:"One",text:"Text"},{title:"Two",text:"Text"}]},{id:"other",title:"Other",cards:[]}]');
 const original=runCode('JSON.stringify(data)');
 runCode('removeCard(data.sections[0],data.sections[0].cards[0])');assert.equal(runCode('data.sections[0].cards.length'),1);
 runCode('removeSection(data.sections[0])');assert.equal(runCode('data.sections.length'),1);
 ...
 context.savedData=savedData;runCode('data=JSON.parse(savedData);dirty=false;location.hash=""'); for(const [k,v]of Object.entries({owner:'NYAYNA',repo:'nyaender',branch:'main'}))get(k).value=v;
 ...
 assert.throws(()=>vm.runInContext('validate({})',context));const bad=structuredClone(content);bad.sections=[{id:'unsafe',title:'Unsafe',cards:[{title:'Unsafe',text:'Text',image:'https://evil.invalid/pixel'}]}];context.bad=bad;assert.throws(()=>vm.runInContext('validate(bad)',context));
